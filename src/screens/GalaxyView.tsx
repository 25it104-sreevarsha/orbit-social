...
export default function GalaxyView() {
  ...
  // Staggered entry delays for the very first mount of the galaxy, ordered
  // by liveAttention descending, spread over ~600ms total. Computed once
  // (lazily, on first render) so later filtering/search never re-triggers
  // the stagger. Skipped entirely when the user prefers reduced motion.
  const entryDelaysRef = useRef<Record<string, number> | null>(null);
  if (entryDelaysRef.current === null) {
    const map: Record<string, number> = {};
    if (!reduced) {
      const sortedByAttention = [...sparks].sort((a, b) => b.liveAttention - a.liveAttention);
      const total = sortedByAttention.length;
      const STAGGER_TOTAL_MS = 600;
      sortedByAttention.forEach((s, i) => {
        map[s.id] = total > 1 ? (i / (total - 1)) * (STAGGER_TOTAL_MS / 1000) : 0;
      });
    }
    entryDelaysRef.current = map;
  }
        ...
        >
          ...
          >
            {constellations.map((c) => {
              ...
              >
                <div className="text-xs font-medium uppercase tracking-widest text-slate-400 -translate-x-1/2 -translate-y-8 text-center whitespace-nowrap">
                  {c.name.replace('-', ' ')}
                </div>
              </div>
              );
            ...
            {filteredSparks.map((s) => {
              ...
              return (
                <GalaxyNode
                  ...
                  isActive={isActive}
                  entryDelay={entryDelaysRef.current?.[s.id] ?? 0}
                />
              );
            })}
          </div>
